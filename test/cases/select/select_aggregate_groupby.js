describe('Test aggregate option with groupby', function () {
    it('select with agregate - min', function (done) {
        con.$sql.run("Select [Min(Price)] from Products group by CategoryID").
    ...
    it('select with agregate - max', function (done) {
        con.$sql.run("Select [maX(Price)] from Products group by CategoryID").
        then(function (results) {
            expect(results).to.be.an('array').length(8);
    ...
    it('select with agregate - sum', function (done) {
        con.$sql.run("Select [sUm(Price)] from Products group by CategoryID").
        then(function (results) {
            expect(results).to.be.an('array').length(8);
    ...
    it('select with agregate - count', function (done) {
        con.$sql.run("Select [counT(Price)] from Products group by CategoryID").
        then(function (results) {
            expect(results).to.be.an('array').length(8);
    ...
    it('select with agregate - avg', function (done) {
        ...
        con.$sql.run("Select [Avg(Price)] from Products group by CategoryID").
        then(function (results) {
            expect(results).to.be.an('array').length(8);
